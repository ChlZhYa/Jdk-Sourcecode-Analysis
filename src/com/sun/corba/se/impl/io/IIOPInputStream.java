...
public class IIOPInputStream
        ...
        if (streamFormatVersion == 2) {
            if (!(orbStream instanceof ValueInputStream)) {
                SystemException sysex = omgWrapper.notAValueinputstream(
                        CompletionStatus.COMPLETED_MAYBE);
                // XXX I18N?  Logging for IOException?
                IOException result = new IOException("Not a ValueInputStream", sysex);
                throw result;
            }
        }
    }
    ...
    public final void reset() throws IOException {
        try {
            ...
            IOException err = new IOException(e.getMessage(), e);
            throw err;
        }
    }
...
// d11623 : implement readFully, required for serializing some core classes
     ...
     * read in the data.  This method is capable of throwing out data not applicable to client's
     * fields.
     * This method handles instances where the reader has a class not sent by the sender, the sender
     * sent
     * a class not present on the reader, and/or the reader's class does not match the sender's
     * class.
     *
     ...
     * However,
     * if either says custom while the other does not, a MARSHAL error is thrown.  Externalizable
     * is
