    ...
    throws MBeanException, RuntimeOperationsException {
        ...
        if (inDescriptorType.equalsIgnoreCase(MMB)) {
            ...
            for (int i=0; i < numAttrs; i++) {
                retList[i] = (attrList[i].getDescriptor());
            }
            ...
            for (int i=0; i < numOpers; i++) {
                retList[i] = (operList[i].getDescriptor());
            }
            ...
            for (int i=0; i < numCons; i++) {
                retList[i] = (consList[i].getDescriptor());
            }
            ...
            for (int i=0; i < numNotifs; i++) {
                retList[i] = (notifList[i].getDescriptor());
            }
            ...
            int j=0;
            for (int i=0; i < numAttrs; i++) {
                retList[j] = (attrList[i].getDescriptor());
                j++;
            }
            for (int i=0; i < numCons; i++) {
                retList[j] = (consList[i].getDescriptor());
                j++;
            }
            for (int i=0; i < numOpers; i++) {
                retList[j] = (operList[i].
                        getDescriptor());
                j++;
            }
            for (int i=0; i < numNotifs; i++) {
                retList[j] = (notifList[i].
                        getDescriptor());
                j++;
            }
        } else {
    ...
    throws MBeanException, RuntimeOperationsException {
        ...
        for (int i=0; (i < numAttrs) && (retInfo == null); i++) {
            if (MODELMBEAN_LOGGER.isLoggable(Level.FINER)) {
                ...
                .append(i).append(":")
                .append(attrList[i].getDescriptor())
                .append("\t\n this.modelMBeanAttributes MBeanAttributeInfo Array ")
                .append(i).append(":")
                .append(modelMBeanAttributes[i].getDescriptor());
                MODELMBEAN_LOGGER.logp(Level.FINER,
                        ModelMBeanInfoSupport.class.getName(),
